    ...
    public function run(): void
    {
        ...
        $cottageType = RoomType::updateOrCreate(
            ...
            [
                ...
                'description' => 'Private cliffside cottages with open verandas overlooking the ocean.',
        ...
        for ($i = 1; $i <= 10; $i++) {
            ...
            $room = Room::updateOrCreate(['slug' => $slug], [
                ...
                'description' => 'Experience absolute tranquility in our premium cliffside cottages. Designed with locally sourced elements, comfortable bedding, modern bathroom fixtures, and a private sea view deck.',
                'max_adults' => 3,
                'max_children' => 2,
                'size_sqft' => 650,
        ...
        ] as [$restaurant_category_id, $name, $description, $price, $signature, $image]) {
            RestaurantItem::updateOrCreate(['restaurant_category_id' => $restaurant_category_id, 'name' => $name], compact('description', 'price') + ['is_signature' => $signature, 'image' => $image, 'is_available' => true]);
        }
        ...
        ] as [$name, $slug, $description, $distance]) {
            Attraction::updateOrCreate(['slug' => $slug], compact('name', 'description', 'distance') + ['is_featured' => true]);
        }
        ...
        ] as [$name, $location, $rating, $quote]) {
            Testimonial::updateOrCreate(['name' => $name], compact('location', 'rating', 'quote') + ['is_featured' => true, 'is_active' => true]);
            Review::updateOrCreate(['name' => $name, 'title' => 'A beautiful stay'], ['rating' => $rating, 'comment' => $quote, 'is_approved' => true]);
        }

        foreach ([
            ['Booking', 'Can I pay online?', 'The platform is ready for Razorpay, Stripe, and PayPal keys. Offline payment is active by default.'],
            ['Stay', 'What are check-in and check-out times?', 'Standard check-in is 2:00 PM and check-out is 11:00 AM. Early access is subject to availability.'],
            ...
            Faq::updateOrCreate(['question' => $question], compact('category', 'answer') + ['is_active' => true]);
        }

        foreach ([
            ['About Us', 'about-us', '<p>Aruvi on the Cliff is a luxury beachside resort designed for travelers who want refined comfort without losing the character of the coast. Every room, dining experience, and service touchpoint is manageable through the admin CMS.</p>'],
            ['Terms & Conditions', 'terms-and-conditions', '<p>Bookings are subject to room availability, guest verification, and resort operating policies. Taxes and payment gateway fees may apply.</p>'],
            ['Privacy Policy', 'privacy-policy', '<p>Guest data is collected only for reservations, communication, compliance, and service personalization. Admin users can manage CMS and operational records securely.</p>'],
            ['Cancellation Policy', 'cancellation-policy', '<p>Cancellation windows and refund rules can be edited from this CMS page. Gateway-specific refunds should follow the selected payment provider policy.</p>'],
            ['Resort Policies', 'resort-policies', '<p>Guests are requested to respect quiet hours, safety guidance, occupancy limits, and property rules shared during confirmation.</p>'],
        ] as [$title, $slug, $content]) {
            CmsPage::updateOrCreate(['slug' => $slug], compact('title', 'content') + ['seo_title' => $title.' - Aruvi on the Cliff', 'seo_description' => strip_tags($content), 'is_published' => true]);
        }
